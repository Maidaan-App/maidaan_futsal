...
type NewsEventsTableComponentProps<T> = {
  data: T[];
  columns: Column<T>[];
  filterTabs: string[];
  statusKey: keyof T;
...
const NewsEventsTableComponent = <
...
>({
  ...
  sortOptions,
  searchKeys,
}: NewsEventsTableComponentProps<T>) => {
  ...
  return (
    <div className={`bg-white  rounded-lg shadow-lg ${poppins.className} p-5`}>
      ...
      <div className="flex flex-col lg:flex-row gap-4 my-5">
        ...
        <Link href={`${paths.admin.addNewsEvents}`}>
          <Button
            variant={"default"}
            ...
            {filteredData.map((item) => (
              ...
              >
                ...
                <td className="px-4 py-2">
                  <Link
                    href={`${paths.admin.editNewsEvents}?id=${item._id}`}
...
export default NewsEventsTableComponent;
